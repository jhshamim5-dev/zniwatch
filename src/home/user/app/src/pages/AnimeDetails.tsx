      ...
      <div className="container -mt-20 sm:-mt-24 relative z-10">
          ...
          >
            <div className="glass rounded-lg sm:rounded-xl p-3 sm:p-4">
              <div className="flex items-center gap-1.5 sm:gap-2 text-muted-foreground mb-0.5 sm:mb-1">
                ...
                <span className="text-[10px] sm:text-xs">Episodes</span>
              </div>
              <p className="font-semibold text-sm sm:text-base">{anime.episodes || 'TBA'}</p>
            </div>
        ...
        {anime.staff?.edges && anime.staff.edges.length > 0 && (
          ...
          <div className="h-px bg-white/5 mb-8 sm:mb-12" />
            ...
            {/* Related Anime */}
            {(() => {
              const related = (anime.relations?.nodes || []).filter((m: any) => m && m.id && m.type !== 'Season' && m.type !== 'RECOMMENDED');
            ...
            {/* Recommended Anime */}
            {(() => {
              const recommended = (anime.relations?.nodes || []).filter((m: any) => m && m.id && m.type === 'RECOMMENDED');
